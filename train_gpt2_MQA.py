...
class GPT(nn.Module):
    ...
    def from_pretrained(cls, model_type):
        ...
        # 注意：MQA版本的注意力结构与标准GPT-2完全不同
        # 标准GPT-2使用MHA（Multi-Head Attention），所有head都有独立的K和V
        # MQA（Multi-Query Attention）所有head共享一个K和V
        # 权重结构不兼容，不支持从标准GPT-2预训练模型加载
        # 如需使用MQA，请从头开始训练
        raise NotImplementedError(
            "MQA模型不支持从标准GPT-2预训练模型加载权重。\n"
            "原因：MQA使用共享的单头Key/Value，而GPT-2使用多头Key/Value，结构不兼容。\n"
            "请使用从头训练：model = GPT(GPTConfig(vocab_size=50304))"
        )
...
def main():
    ...
    if master_process:
        ...
        print(f"[MQA] 使用多查询注意力（Multi-Query Attention）- Query多头，Key/Value单头共享")
    ...
    log_dir = "/opt/train/data/nanogpt/MQA/log"
