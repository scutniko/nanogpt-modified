...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # ALiBi位置编码
        self.alibi = ALiBi(n_head=config.n_head, max_seq_len=config.block_size)
    ...
    def forward(self, x):
        ...
        # 获取ALiBi偏置 [1, n_head, T, T]
        alibi_bias = self.alibi(T)

        # 使用带ALiBi偏置的attention计算
        # 注意：F.scaled_dot_product_attention的attn_mask参数可以接受额外的偏置
        # is_causal=True表示是因果注意力，即只能看到前面的token，不能看到后面的token。
        # 返回的y的shape是[B, n_head, T, head_size]
        y = F.scaled_dot_product_attention(q, k, v, attn_mask=alibi_bias, is_causal=True)
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # 注意：ALiBi版本不需要位置编码embedding，位置信息通过attention bias编码
            # blocks
    ...
    def forward(self, idx, targets=None):
        ...
        # 只获取token embedding，不需要位置编码（ALiBi通过attention bias处理）
    ...
    def from_pretrained(cls, model_type):
        ...
        # 注意：ALiBi版本的模型结构与原始GPT-2不同，不能直接加载预训练权重
        # 这里保留接口但会跳过位置编码相关的权重
        print("Warning: ALiBi model cannot load positional embeddings from standard GPT-2")
...
# -----------------------------------------------------------------------------
# simple launch:
# python train_gpt2_ALiBi.py
# DDP launch for e.g. 8 GPUs:
# torchrun --standalone --nproc_per_node=8 train_gpt2_ALiBi.py
...
def main():
    ...
    if master_process:
        ...
        print(f"[ALiBi] 使用线性偏置注意力（Attention with Linear Biases）")
    ...
    # model = GPT.from_pretrained("gpt2") # 注意：ALiBi版本不能直接加载GPT-2预训练权重
    ...
    log_dir = "/opt/train/data/nanogpt/alibi/log"
