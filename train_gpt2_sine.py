...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # 标准正弦位置编码（固定，不可学习）
            wpe = SinusoidalPositionalEncoding(config.n_embd, config.block_size),
            # blocks
    ...
    def forward(self, idx, targets=None):
        ...
        # 获取token embedding和位置编码
        tok_emb = self.transformer.wte(idx) # shape (B, T, n_embd)
        pos_emb = self.transformer.wpe(tok_emb) # shape (B, T, n_embd)
        # 将token embedding和位置编码相加
        x = tok_emb + pos_emb
    ...
    def from_pretrained(cls, model_type):
        ...
        # 注意：正弦位置编码版本的模型结构与原始GPT-2不同
        # GPT-2使用可学习的位置编码，而此版本使用固定的正弦位置编码
        # 位置编码参数不会被加载，只加载其他权重
        print("Warning: Sinusoidal PE model uses different positional encoding than standard GPT-2")
...
# -----------------------------------------------------------------------------
# simple launch:
# python train_gpt2_sine.py
# DDP launch for e.g. 8 GPUs:
# torchrun --standalone --nproc_per_node=8 train_gpt2_sine.py
...
def main():
    ...
    if master_process:
        ...
        print(f"[Sinusoidal PE] 使用标准正弦位置编码（Sinusoidal Positional Encoding）")
    ...
    # model = GPT.from_pretrained("gpt2") # 注意：正弦位置编码版本使用固定位置编码，与GPT-2的可学习位置编码不同
